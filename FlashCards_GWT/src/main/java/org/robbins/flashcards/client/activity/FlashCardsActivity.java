	...
	public FlashCardsActivity(ClientFactory clientFactory) {
		...
		getRegistrations().add(LoadFlashCardEvent.register(this.eventBus, new LoadFlashCardEventHandler() {
			...
			public void onLoadFlashCard(LoadFlashCardEvent event) {
				...
				FlashCardsActivity.this.placeController.goTo(new EditFlashCardPlace(Long.toString(event.getFlashCardId())));
			}
		}));
		...
		getRegistrations().add(LoadTagEvent.register(this.eventBus, new LoadTagEventHandler() {
			...
			public void onLoadTag(LoadTagEvent event) {
				...
				FlashCardsActivity.this.placeController.goTo(new EditTagPlace(Long.toString(event.getTagId())));
			}
		}));
		
		getRegistrations().add(DeleteFlashCardEvent.register(this.eventBus, new DeleteFlashCardEventHandler() {
			@Override
			public void onDeleteFlashcard(DeleteFlashCardEvent event) {
				GWT.log("FlashCardsActivity: 'Delete FlashCard' event");
				
				flashCardService.deleteFlashCards(ConstsUtil.DEFAULT_AUTH_HEADER, event.getFlashCardId(), new MethodCallback<java.lang.Void>() {
