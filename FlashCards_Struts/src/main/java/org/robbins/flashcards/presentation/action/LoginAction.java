...
public class LoginAction extends FlashCardsAppBaseAction implements Preparable,
    ...
    // we'll need access to the Servlet spec objects, rather than just their attribute or
    // parm maps
    ...
    // the OpenIdAuthenticator class needs access to the application to store a
    // OpenId4Java related object
    private Map<String, Object> application;

    // we'll need to send this to the OpenId provider so it knows where to send its
    // response
    ...
    public String validateOpenId() throws Exception {
        ...
        if (getOpenid_identifier().endsWith("/")) {
            setOpenid_identifier(getOpenid_identifier().substring(0,
        ...
        String destinationUrl = OpenId4JavaAuthenticator.getValidateOpenIdUrl(returnUrl,
                this.getOpenid_identifier(), httpSession, application);
