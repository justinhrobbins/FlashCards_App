...
public class PartialResponseFilter implements ResponseHandler {
	...
	public Response handleResponse(Message m, OperationResourceInfo ori,
		...
		if (!ori.getHttpMethod().equals("GET")) return null;
		
		// exit now if not a 200 response, no sense in apply filtering if not a '200 OK'
		if (response.getStatus() != 200) return null;
		
        // exit now if we are not returning json
        if (!ori.getProduceTypes().get(0).toString().equals(MediaType.APPLICATION_JSON)) return null;

		// get a reference to the response entity.  the entity holds the payload of our response
		Object entity = response.getEntity();
		
		// try to get the 'fields' parameter from the QueryString
		String fields = uriInfo.getQueryParameters().getFirst("fields");
		
		// if the 'fields' QueryString is blank, then check to see if we have @DefaultValue for 'fields' 
		if (StringUtils.isBlank(fields)) {
			...
			for (Parameter parm : parameters) {
				...
				if (parm.getName().equals("fields")) {
					...
					// now that we found 'fields', there's no need to keep looping 
					break;
				}
			}
	...
	// According to the Jackson docs, "ObjectWriter instances are immutable and thread-safe: they are created by ObjectMapper"
	// You should not change config settings directly on the ObjectMapper while using it (changing config of ObjectMapper is not thread-safe
	private Response applyWriter(Object object, SimpleFilterProvider filterProvider, Response response) {
		
		try {
			String jsonString = objectMapper.writer(filterProvider).writeValueAsString(object);
	...
	private Set<String> getFieldsAsSet(String fields) {
	    ...
	    while (st.hasMoreTokens()) {
	    	...
	    	// never allow 'userpassword' to be passed even if it was specifically requested
	    	if (field.equals("userpassword")) continue;
	...
	private void initializeEntity(Object entity, Set<String> fields) {
	    ...
	    for (String field : fields) {
        	try {
        		...
        		// This is needed since Hibernate will not auto-initialize most collections
        		// Therefore, if we want to return the field in the response, we need to make sure it is loaded
